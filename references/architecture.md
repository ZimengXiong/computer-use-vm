# Codex VM Bridge Architecture

The bridge has three parts:

1. Host CLI/MCP server in `codex_vm_bridge`.
...
```text
Codex -> MCP stdio -> host bridge -> guest HTTP agent -> native macOS APIs
```
...
Local prepared base: `codex-vm-computer-base`.

This base was created from `codex-pppc-test` after macOS privacy prompts were approved inside the VM. It includes `/Users/admin/codex-vm-agent`, a compiled Swift helper, and the LaunchAgent `/Users/admin/Library/LaunchAgents/local.codex.vm-agent.plist`, which starts the HTTP agent on port 7042 in the logged-in GUI session. A verification clone successfully returned a native screenshot through direct HTTP at `/tmp/codex-base-verify-native.png`.
