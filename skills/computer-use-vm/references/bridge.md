...
Fallback backend: UTM. It supports `utmctl start --hide`, `--disposable`, `clone`, `exec`, guest file push/pull, and IP lookup. UTM's source also exposes SPICE input commands, but the computer-use-vm bridge should prefer the guest agent for performance.
